...
namespace Foodcore.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            var result = dc.Products.ToList();
            var result1 = dc.Carts.ToList();

            var res = from t in result
                      join t1 in result1
                      on t.Productid equals t1.Itemid
                      where (t1.Email == HttpContext.Session.GetString("login_eamil"))
                      select t;
        ...
        {
            var result = dc.Products.ToList();
            var getproduct = dc.Products.Where(s => s.Productname.Contains(search)).ToList();
